...
import { listRooms, listClassSchedules } from '../utils/classSchedules.js';
  ...
  asyncHandler(async (_req, res) => {
    const { rows } = await query(
      `SELECT s.id, s.name, s.slug, s.description,
              ...
              EXISTS (
                SELECT 1 FROM studio_owners so WHERE so.studio_id = s.id
              ) AS has_owner
         FROM studios s
...
studiosRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [teachers, rooms, classSchedules] = await Promise.all([
      query(
